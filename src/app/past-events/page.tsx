...
import { type ArchivedEvent, type SiteContent } from "@/lib/content-types";
...
export default function PastEventsPage() {
  ...
  useEffect(() => { fetch("/api/content").then((response) => response.json()).then((content: SiteContent) => setEvents(content.events)).catch(() => undefined); }, []);
  ...
  return (
    <ChapterShell current="ARCHIVE">
      ...
      <section id="archive" className="archive-section"><div className="archive-head scroll-reveal"><p className="section-tag">( 01 / PAST EVENTS )</p><p>Every event leaves a trace. Pick one to read the room.</p></div><div className="archive-list">{events.map((event) => <button className="archive-event scroll-reveal" onClick={() => setSelected(event)} key={event.id}><span>{event.date}</span><small>{event.type}</small><b>{event.title}</b><ArrowUpRight /></button>)}</div></section>
